...
isAdminLogin();
if (isset($_POST['getBookingRecords'])) {
    ...
    $values = ["%$frm[search]%", "%$frm[search]%", "%$frm[search]%", "%$frm[search]%", "%$frm[search]%"];
    $res = select($q, $values, "sssss");

    $limit = 4;
    $page = $frm['page'];
    $start = ($page - 1) * $limit;
    $limit_q = $q . " LIMIT $start, $limit";
    $limit_res = select($limit_q, $values, "sssss");
    $bookingrecords_data = "";
    if ($limit_res->num_rows > 0) {
        ...
        while ($row = mysqli_fetch_assoc($limit_res)) {
            ...
            }else if($row['booking_status'] == "failed"){
                $status = "bg-danger";
            ...
            $bookingrecords_data .= "
                <tr>
                    ...
                    <td class='text-center'>
                        <span class='badge w-100 $status'>
                            $status_msg
                        </span>
                    </td>
                    <td class='text-center'>
                        <button type='button' onclick='generate_pdf($row[booking_id])' class='btn w-75 shadow-none btn-outline-primary'>
        ...
        if ($total_rows > $limit) {
            ...
            $disabled = ($page == 1) ? "<li class='page-item disabled'><button class='shadow-none page-link'>&laquo;</button></li>" : "<li class='page-item' onclick='changePage($prev)' ><button class='shadow-none page-link'>&laquo;</button></li>";
            ...
            $disabled = ($page == $total_number_of_pages) ? "<li class='page-item disabled'><button class='shadow-none page-link'>&raquo;</button></li>" : "<li class='page-item' onclick='changePage($next)' ><button class='shadow-none page-link'>&raquo;</button></li>";
        ...
        echo json_encode(['bookingrecords_data' => $bookingrecords_data, 'pagination' => $pagination]);
    } else {
        $bookingrecords_data = "
            <tr>
