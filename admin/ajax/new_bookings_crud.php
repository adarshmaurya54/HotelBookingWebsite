...
isAdminLogin();
if (isset($_POST['getNewBookings'])) {
    $frm = filteration($_POST);
    $q = "SELECT * FROM `booking_order` bo 
    INNER JOIN `booking_details` bd ON bo.booking_id=bd.booking_id
     WHERE (bo.booking_status=? AND bo.arrival=?)
    ...
    $values = ["booked", 0, "%$frm[search]%", "%$frm[search]%", "%$frm[search]%", "%$frm[search]%", "%$frm[search]%"];
    ...
    if ($limit_res->num_rows > 0) {
            ...
            $newbookings_data .= "
                <tr>
                    ...
                    <td>
                        ...
                        <br>
                        <span><strong>Paid</strong> : ₹$row[total_pay].00</span>
                        <br>

                    </td>
                    <td class='text-center'>
                        <button type='button' onclick='assing_booking_id($row[booking_id],$deletePage)' class='btn w-75 shadow-none btn-outline-primary' data-bs-toggle='modal' data-bs-target='#assignRoom'>
                           <i class='bi bi-building-fill-check'></i> Assign Room
                        </button>
                        <br>
                        <button type='button' onclick='cancelBooking($row[booking_id],$deletePage)' class='mt-2 w-75 shadow-none btn btn-outline-danger'>
                           <i class='bi bi-x-square-fill'></i> Cancel Booking
                        </button>
                    </td>
                </tr>
            ";
        ...
        echo json_encode(['newbookings_data' => $newbookings_data, 'pagination' => $pagination]);
    } else {
        $newbookings_data = "
            <tr>
...
} else if (isset($_POST['assingRoom'])) {
    $frm = filteration($_POST);
    $q = "UPDATE `booking_order` bo INNER JOIN `booking_details` bd ON bo.booking_id = bd.booking_id
     SET bo.arrival=?,bo.review_and_rating=?, bd.room_no=? WHERE bo.`booking_id`=?";
    $values = [1,0,$frm['room_no'], $frm['booking_id']];
    $res = update($q, $values, 'iisi'); // it will affect two tables
    echo $res;
} else if (isset($_POST['cancelBooking'])) {
    $frm = filteration($_POST);
    $upd_q = "UPDATE `booking_order` SET `booking_status`=?,`refund`=? WHERE `booking_id`=?";
    $value = ['cancelled',0,$frm['id']];
    $res = update($upd_q,$value,"sii");
    echo $res;
}
